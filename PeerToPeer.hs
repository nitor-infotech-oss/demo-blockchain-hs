...
broadCastBlock tVarBlockChain tVarPeers = do
    ...
    where
        fn ip port blockChain = sendAllMsg ip port (serializeMessage (ReceiveLatestBlock (last blockChain)))
...
broadCastBlockChain tVarBlockChain tVarPeers = do
    ...
    mapM_ (\(ip,port) -> fn ip port blockChain) (Set.toList peers)
    where
        fn ip port blockChain = sendAllMsg ip port (serializeMessage (ReceiveLatestBlockChain blockChain))
...
requestAndAddLatestBlock tVarBlockChain peerIp port = do
    result <- E.try (getClientSocket peerIp port) :: IO (Either E.SomeException Socket)
    case result of
      Left _ -> return ()
      Right sock -> do
...
sendAllMsg peerIp peerPort  msg = do
  result <- E.try (getClientSocket peerIp peerPort) :: IO (Either E.SomeException Socket)
  case result of
    Left _     -> return ()
    Right sock -> sendAll sock msg
